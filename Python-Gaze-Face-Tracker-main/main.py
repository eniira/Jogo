...
#Codigo com alterações e adaptações 
...
try:
    ...
    aux_face = None #Adaptado Catarine
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            z = angles[2] * 360
            print("angle_x = ", angle_x) #Adaptado Catarine
            ...
            if angle_y < -threshold_angle:
                ...
                face_looks = "Right"
            elif angle_x < 0: #Adaptado Catarine 
                face_looks = "Down"
            ...
            if PRINT_DATA: #Comentado Catarine
                ...
                if ENABLE_HEAD_POSE:
                    ...
                    #if PRINT_DATA:
                     #   print(f"Head Pose Angles: Pitch={pitch}, Yaw={yaw}, Roll={roll}") #Comentado Catarine
            # Logging data
            if LOG_DATA:
            ...
            # Sending data through socket #Comentado Catarine
            #timestamp = int(time.time() * 1000)  # Current timestamp in milliseconds
            ...
            if aux_face != face_looks: #Adicionado Catarine
                SERVER_ADDRESS = ("127.0.0.1", 7070)
                iris_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                iris_socket.sendto(face_looks.encode(), SERVER_ADDRESS)
                aux_face = face_looks

            #print(f'Sent UDP packet to {SERVER_ADDRESS}: {face_looks}') #Comentado Catarine
